...
const getToken = async (code: string) => {
  ...
  if (response.ok) {
    setCookie('yandexToken', `${data.access_token}`, { maxAge: 60 * 60 });
    console.log(data)
...
export default async function Auth({ searchParams }: { searchParams: { [key: string]: string | string[] | undefined } }) {
  // // Server Actions for cookies
  // async function setTokenCookies(value: string) {
  //   'use server'
  //   if (!value) return;

  //   cookies().set('yandexToken', `${value}`, {maxAge: 3600});
